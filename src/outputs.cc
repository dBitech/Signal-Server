...
void DoPathLoss(char *filename, unsigned char geo, unsigned char kml, unsigned char ngs, struct site *xmtr,
  ...
  if (debug) {
    fprintf(stderr, "\nWriting \"%s\" (%ux%u pixmap image)...\n", filename != nullptr ? mapfile : "to stdout", width,
            (kml ? height : height + 30));
    fflush(stderr);
  }
      ...
      if (found) {
        ...
        if (loss <= region.level[0])
          ...
          for (z = 1; (z < region.levels && match == 255); z++) {
            if (loss >= region.level[z - 1] && loss < region.level[z]) match = z;
          }
        }
    ...
    for (x = 0, lon = max_west; x < (int)width; x++, lon = max_west - (dpp * (double)x)) {
      ...
      if (found) {
        ...
        if (signal >= region.level[0])
          ...
          for (z = 1; (z < region.levels && match == 255); z++) {
            if (signal < region.level[z - 1] && signal >= region.level[z]) match = z;
          }
        }
            ...
            if (red != 0 || green != 0 || blue != 0)
              ...
              if (ngs)
                ...
                if (dem[indx].data[x0][y0] == 0)
                  ...
                  /* Elevation: Greyscale */
                  terrain =
                      (unsigned)(0.5 + pow((double)(dem[indx].data[x0][y0] - min_elevation), one_over_gamma) * conversion);
                  ADD_PIXEL(&ctx, terrain, terrain, terrain);
                }
              }
            }
    ...
    for (x = 0, lon = max_west; x < (int)width; x++, lon = max_west - (dpp * (double)x)) {
      ...
      if (found) {
        ...
        if (dBm >= region.level[0])
          ...
          for (z = 1; (z < region.levels && match == 255); z++) {
            if (dBm < region.level[z - 1] && dBm >= region.level[z]) match = z;
          }
        }
...
void PathReport(struct site source, struct site destination, char *name, char graph_it, int propmodel, int pmenv,
  ...
  if (LR.frq_mhz > 0.0) {
      ...
      switch (propmodel) {
        case 1:
          ...
          point_to_point_ITM(source.alt * METERS_PER_FOOT, destination.alt * METERS_PER_FOOT, LR.eps_dielect,
                             LR.sgm_conductivity, LR.eno_ns_surfref, LR.frq_mhz, LR.radio_climate, LR.pol, LR.conf, LR.rel,
                             loss, strmode, errnum);
          break;
        case 3:
          // HATA 1, 2 & 3
        ...
        case 8:
          // ITWOM 3.0
          point_to_point(source.alt * METERS_PER_FOOT, destination.alt * METERS_PER_FOOT, LR.eps_dielect, LR.sgm_conductivity,
    ...
    if ((loss * 1.5) < free_space_loss) {
      fprintf(fd2,
              "Model error! Computed loss of %.1fdB is greater than free space loss of %.1fdB. Check your inuts for model %d\n",
  ...
  fprintf(stderr, "Path loss (dB), Received Power (dBm), Field strength (dBuV):\n%.1f\n%.1f\n%.1f", loss, dBm, field_strength);
...
void SeriesData(struct site source, struct site destination, char *name, unsigned char fresnel_plot, unsigned char normalised) {
  ...
  struct site remote;
  FILE *fd = nullptr, *fd1 = nullptr, *fd2 = nullptr, *fd3 = nullptr, *fd4 = nullptr, *fd5 = nullptr;

  ReadPath(destination, source);           // BUG: ARE THESE ARGS FLIPPED?
  azimuth = Azimuth(destination, source);  // BUG: ARE THESE ARGS FLIPPED?
  distance = Distance(destination, source);
  refangle = ElevationAngle(destination, source);  // BUG: ARE THESE ARGS FLIPPED?
  ...
  if (clutter > 0.0) fd1 = fopen(cluttername, "wb");
  ...
  for (x = 0; x < path.length - 1; x++) {
    ...
    if (metric) {
      ...
      if (fd1 != nullptr && x > 0 && x < path.length - 2)
        fprintf(fd1, "%.3f %.3f\n", KM_PER_MILE * path.distance[x],
                METERS_PER_FOOT * (terrain == 0.0 ? height : (height + clutter)));
    ...
    if ((height + clutter) > maxheight) maxheight = height + clutter;
