...
void PlotLOSPath(struct site source, struct site destination, char mask_value, FILE *fd) {
  ...
  for (y = 0; (y < (path.length - 1) && path.distance[y] <= max_range); y++) {
    ...
    if ((GetMask(path.lat[y], path.lon[y]) & mask_value) == 0 && can_process(path.lat[y], path.lon[y])) {
...
void PlotPropPath(struct site source, struct site destination, unsigned char mask_value, FILE *fd, int propmodel, int knifeedge,
  ...
  double loss, azimuth, pattern = 0.0, xmtr_alt, dest_alt, xmtr_alt2, dest_alt2, cos_rcvr_angle, cos_test_angle = 0.0, test_alt,
                        elevation = 0.0, distance = 0.0, four_thirds_earth, field_strength = 0.0, rxp, dBm, diffloss;
  struct site temp;
  float dkm;
  ...
  for (y = 2; (y < (path.length - 1) && path.distance[y] <= max_range); y++) {
    ...
    if ((GetMask(path.lat[y], path.lon[y]) & 248) != (mask_value << 3) && can_process(path.lat[y], path.lon[y])) {
      char fd_buffer[64];
      int buffer_offset = 0;
      ...
      switch (propmodel) {
        ...
        case 11:
          ...
          loss = EgliPathLoss(LR.frq_mhz, source.alt * METERS_PER_FOOT,
                              (path.elevation[y] * METERS_PER_FOOT) + (destination.alt * METERS_PER_FOOT), dkm);
          break;
        case 12:
          // Soil
...
void PlotPropagation(struct site source, double altitude, char *plo_filename, int propmodel, int knifeedge, int haf, int pmenv,
  ...
  if (debug) {
    fprintf(stderr, " contours of \"%s\" out to a radius of %.2f %s with Rx antenna(s) at %.2f %s AGL\n", source.name,
            metric ? max_range * KM_PER_MILE : max_range, metric ? "kilometers" : "miles",
