...
double fht(const double &x, const double &pk) {
  ...
  if (x < 200.0) {
    ...
    if (pk < 1.0e-5 || x * w * w * w > 5495.0) {
      ...
      if (x > 1.0) fhtv = 40.0 * log10(x) + fhtv;
    } else
...
double h0f(double r, double et) {
  ...
  if (q != 0.0) h0fv = (1.0 - q) * h0fv + q * 4.343 * log((a[it] * x + b[it]) * x + 1.0);
    ...
    if (prop.tgh > prop.cch) { /* for TX ant above all clutter height */
      ...
      if (d1a < 50.0) {
        ...
        if (d1a < 225.0) {
          ...
          arte = q + (0.7 * pdk - std::max(0.01, log10(prop.wn * 47.7) - 2)) * (prop.hg[1] / hone);
        }
          ...
          if (pdk > zi) {
            q = (pdk - zi) * 10.2 * ((sqrt(std::max(0.01, log10(prop.wn * 47.7) - 2.0))) / (100 - zi));
          } else {
            q = ((zi - pdk) / zi) * (-20.0 * std::max(0.01, log10(prop.wn * 47.7) - 2.0)) / sqrt(hone);
          }
          arte = arte + q;
        }
      }
      ...
      arte = arte - (std::max(0.01, log10(prop.wn * 47.7) - 2)) * (prop.hg[1] / prop.tgh);
    }
    saalosv = arte;
  }
  return saalosv;
...
double adiff(double d, prop_type &prop, propa_type &propa) {
  ...
  if (d == 0) {
    ...
    afo = std::min(15.0, 2.171 * log(1.0 + 4.77e-4 * prop.hg[0] * prop.hg[1] * prop.wn * q));
    ...
    q = (wd1 + xd1 / d) * std::min(((1.0 - 0.8 * exp(-d / 50e3)) * prop.dh * prop.wn), 6283.2);
    wd = 25.1 / (25.1 + sqrt(q));
...
double adiff2(double d, prop_type &prop, propa_type &propa) {
  ...
  double a, q, pk, rd, ds, dsl, /* dfdh, */ th, wa, /* ar, wd, sf1, */
      sf2, /* ec, */ vv, kedr = 0.0, arp = 0.0, sdr = 0.0, pd = 0.0, srp = 0.0, kem = 0.0, csd = 0.0, sdl = 0.0, adiffv2 = 0.0,
                         closs = 0.0;
...
double ascat(double d, prop_type &prop, propa_type &propa) {
  ...
  if (d == 0.0) {
    ...
    if (h0s > 15.0)
      ...
      if (r1 < 0.2 && r2 < 0.2) return 1001.0;  // <==== early return
      ...
      h0 += std::min(h0, (1.38 - log(ett)) * log(ss) * log(q) * 0.49);
      h0 = std::fdim(h0, 0.0);
    ...
    ascatv =
        ahd(th * d) + 4.343 * log(47.7 * prop.wn * (th * th * th * th)) - 0.1 * (prop.ens - 301.0) * exp(-th * d / 40e3) + h0;
  }
...
void qlrps(double fmhz, double zsys, double en0, int ipol, double eps, double sgm, prop_type &prop) {
  ...
  if (zsys != 0.0) prop.ens *= exp(-zsys / 9460.0);
  ...
  if (ipol != 0.0) prop_zgnd = prop_zgnd / zq;
...
double alos(double d, prop_type &prop, propa_type &propa) {
  ...
  if (d == 0.0) {
    ...
    sps = q / sqrt(d * d + q * q);
    r = (sps - prop_zgnd) / (sps + prop_zgnd) * exp(-std::min(10.0, prop.wn * s * sps));
    q = abq_alos(r);

    if (q < 0.25 || q < sps) r = r * sqrt(sps / q);
  ...
  if (d == 0.0) {
    ...
    if (prop.mdp < 0) {
      ...
      if ((sps < 0.05) && (prop.cch > hrg) &&
          (prop.dist < prop.dl[0])) { /* if far from transmitter and receiver below canopy */
        cd = std::max(0.01, pd * (prop.cch - hrg) / (htg - hrg));
        cr = std::max(0.01, pd - dr + dr * (prop.cch - drh) / htg);
        q = ((1.0 - 0.8 * exp(-pd / 50e3)) * prop.dh * (std::min(-20 * log10(cd / cr), 1.0)));
      }
    }
...
void lrprop(double d, prop_type &prop, propa_type &propa) {
  ...
  if (prop.mdp != 0) {
    for (j = 0; j < 2; j++) propa.dls[j] = sqrt(2.0 * prop.he[j] / prop.gme);
    ...
    if (prop.wn < 0.838 || prop.wn > 210.0) prop.kwx = std::max(prop.kwx, 1);

    for (j = 0; j < 2; j++)
      if (prop.hg[j] < 1.0 || prop.hg[j] > 1000.0) prop.kwx = std::max(prop.kwx, 1);

    for (j = 0; j < 2; j++)
      if (abs(prop.the[j]) > 200e-3 || prop.dl[j] < 0.1 * propa.dls[j] || prop.dl[j] > 3.0 * propa.dls[j])
        prop.kwx = std::max(prop.kwx, 3);

    if (prop.ens < 250.0 || prop.ens > 400.0 || prop.gme < 75e-9 || prop.gme > 250e-9 ||
        prop_zgnd.real() <= abs(prop_zgnd.imag()) || prop.wn < 0.419 || prop.wn > 420.0)
      prop.kwx = 4;
  ...
  if (prop.dist > 0.0) {
    if (prop.dist > 1000e3) prop.kwx = std::max(prop.kwx, 1);

    if (prop.dist < dmin) prop.kwx = std::max(prop.kwx, 3);
  ...
  if (prop.dist < propa.dlsa) {
    if (!wlos) {
      ...
      if (d0 < d1) {
        ...
        q = log(d2 / d0);
        propa.ak2 = std::max(0.0, ((d2 - d0) * (a1 - a0) - (d1 - d0) * (a2 - a0)) / ((d2 - d0) * log(d1 / d0) - (d1 - d0) * q));
    ...
    if (prop.dist > 0.0) prop.aref = propa.ael + propa.ak1 * prop.dist + propa.ak2 * log(prop.dist);
  }
  ...
  if (prop.dist <= 0.0 || prop.dist >= propa.dlsa) {
    if (!wscat) {
      ...
      if (a5 < 1000.0) {
        ...
        propa.dx = std::max(propa.dlsa, std::max(propa.dla + 0.3 * xae * log(47.7 * prop.wn),
...
void lrprop2(double d, prop_type &prop, propa_type &propa) {
  ...
  if (prop.mdp != 0) { /* if oper. mode is not 0, i.e. not area mode ongoing */
    ...
    if (abs(prop.the[0]) > 200e-3) prop.kwx = std::max(prop.kwx, 3);

    if (abs(prop.the[1]) > 1.220) prop.kwx = std::max(prop.kwx, 3);
      ...
      if (!wlos) {
        ...
        if (propa.aed > 0.0) {
          ...
          if (d0 < d1) {
            ...
            a2 = std::min(a2, alos2(d2, prop, propa));
            q = log(d2 / d0);
            propa.ak2 =
                std::max(0.0, ((d2 - d0) * (a1 - a0) - (d1 - d0) * (a2 - a0)) / ((d2 - d0) * log(d1 / d0) - (d1 - d0) * q));
            wq = propa.aed >= 0.0 || propa.ak2 > 0.0;
          ...
          if (!wq) {
            propa.ak1 = std::fdim(a2, a1) / (d2 - d1);
            propa.ak2 = 0.0;
...
double avar(double zzt, double zzl, double zzc, prop_type &prop, propv_type &propv) {
  ...
  static __thread double dexa, de, vmd, vs0, sgl, sgtm, sgtp, sgtd, tgtd, gm, gp, cv1, cv2, yv1, yv2, yv3, csm1, csm2, ysm1,
      ysm2, ysm3, csp1, csp2, ysp1, ysp2, ysp3, csd1, zd, cfm1, cfm2, cfm3, cfp1, cfp2, cfp3;
  ...
  if (fabs(zt) > 3.1 || fabs(zl) > 3.1 || fabs(zc) > 3.1) prop.kwx = std::max(prop.kwx, 1);
  ...
  if (avarv < 0.0) avarv = avarv * (29.0 - avarv) / (29.0 - 10.0 * avarv);
    ...
    if (!wq) {
      for (i = 1; i < np; i++) {
        ...
        if (q > 0.0) {
          ...
          prop.the[1] = std::max(prop.the[1], -1.568);
          prop.hhr = pfl[np + 2 - i];
...
void z1sq1(double z[], const double &x1, const double &x2, double &z0, double &zn) {
  ...
  xn = z[0];
  xa = int(std::fdim(x1 / z[1], 0.0));
  xb = xn - int(std::fdim(xn, x2 / z[1]));
...
double d1thx(double pfl[], const double &x1, const double &x2) {
  ...
  kb = n - ka + 1;
  sn = n - 1;
  s = new double[n + 2];  // Throws on fail.
  s[0] = sn;
  s[1] = 1.0;
  xb = (xb - xa) / sn;
  k = (int)(xa + 1.0);
...
void qlrpfl(double pfl[], int klimx, int mdvarx, prop_type &prop, propa_type &propa, propv_type &propv) {
  ...
  for (j = 0; j < 2; j++) xl[j] = std::min(15.0 * prop.hg[j], 0.1 * prop.dl[j]);
  ...
  if (prop.dl[0] + prop.dl[1] > 1.5 * prop.dist) {
    ...
    for (j = 0; j < 2; j++)
      prop.dl[j] = sqrt(2.0 * prop.he[j] / prop.gme) * exp(-0.07 * sqrt(prop.dh / std::max(prop.he[j], 5.0)));
...
void qlrpfl2(double pfl[], int klimx, int mdvarx, prop_type &prop, propa_type &propa, propv_type &propv) {
  ...
  if ((np < 1) || (pfl[1] > 150.0)) {
    ...
    prop.thenr = atan(std::max(0.0, (pfl[np + 2] - pfl[np + 1])) / pfl[1]);
  }
...
void point_to_point_ITM(double tht_m, double rht_m, double eps_dielect, double sgm_conductivity, double eno_ns_surfref,
                        double frq_mhz, int radio_climate, int pol, double conf, double rel, double &dbloss, char *strmode,
                        int &errnum)
...
void point_to_point(double tht_m, double rht_m, double eps_dielect, double sgm_conductivity, double eno_ns_surfref,
                    double frq_mhz, int radio_climate, int pol, double conf, double rel, double &dbloss, char *strmode,
                    int &errnum)
