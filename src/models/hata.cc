...
*/
  ...
  if (f<200) {
      lh_M = std::log10(1.54 * h_M);
      C_H = 8.29 * (lh_M * lh_M) - 1.1;
  } else {
      lh_M = std::log10(11.75 * h_M);
      C_H = 3.2 * (lh_M * lh_M) - 4.97;
  }

  float L_u = 69.55 + 26.16 * logf - 13.82 * std::log10(h_B) - C_H + (44.9 - 6.55 * std::log10(h_B)) * std::log10(d);
