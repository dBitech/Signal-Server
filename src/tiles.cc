...
int tile_load_lidar(tile_t *tile, const char *filename) {
  ...
  // Positive westing
  tile->width_deg =
      tile->max_west - tile->min_west >= 0 ? tile->max_west - tile->min_west : tile->max_west + (360 - tile->min_west);
...
int tile_rescale(tile_t *tile, float scale) {
  ...
  if (debug) {
    fprintf(stderr, "Resampling tile %s [%.1f]:\n\tOld %dx%d. New %zux%zu\n\tScale %f Skip %zu Copy %zu\n", tile->filename,
            tile->resolution, tile->width, tile->height, new_width, new_height, scale, skip_count, copy_count);
    fflush(stderr);
  }
