...
double earthradius, max_range = 0.0, forced_erp, dpp, ppd, yppd, fzone_clearance = 0.6, forced_freq, clutter, lat, lon, txh,
                    tercon, terdic, north, east, south, west, dBm, loss, field_strength, min_north = 90, max_north = -90,
                    min_west = 360, max_west = -1, westoffset = 180, eastoffset = -180, delta = 0, rxGain = 0, antenna_rotation,
                    antenna_downtilt, antenna_dt_direction, cropLat = -70, cropLon = 0, cropLonNeg = 0;

int ippd, mpi, max_elevation = -32768, min_elevation = 32768, bzerror, gzerr, contour_threshold, pred, pblue, pgreen, ter,
               multiplier = 256, debug = 0, loops = 100, jgets = 0, MAXRAD, hottest = 0, height, width, resample = 0,
...
double GetElevation(struct site location) {
  ...
  for (indx = 0, found = 0; indx < MAXPAGES && found == 0;) {
    if (x >= dem[indx].min_x && x < dem[indx].max_x && y >= dem[indx].min_y && y < dem[indx].max_y)
      found = 1;
    else
      indx++;
  }
...
int AddElevation(double lat, double lon, double height, int size) {
  ...
  if (found && size < 2) dem[indx].data[x][y] += (short)rint(height);
  ...
  if (found && size > 1) {
    for (i = size * -1; i <= size; i = i + 1) {
      for (j = size * -1; j <= size; j = j + 1) {
        if (x + j >= 0 && x + j <= IPPD && y + i >= 0 && y + i <= IPPD) dem[indx].data[x + j][y + i] += (short)rint(height);
      }
    }
  }
...
void ReadPath(struct site source, struct site destination) {
  ...
  for (distance = 0.0, c = 0; (total_distance != 0.0 && distance <= total_distance && c < ARRAYSIZE);
    ...
    if (path.elevation[c] == 0 && path.elevation[c - 1] > 10) path.elevation[c] = path.elevation[c - 1];
...
void ObstructionAnalysis(struct site xmtr, struct site rcvr, double f, FILE *outfile) {
  ...
  int x;
  struct site site_x;
  double h_r, h_t, h_x, h_r_orig, cos_tx_angle, cos_test_angle, cos_tx_angle_f1, cos_tx_angle_fpt6, d_tx, d_x, h_r_f1, h_r_fpt6,
      h_f, h_los, lambda = 0.0;
      ...
      if (site_x.lat >= 0.0) {
        if (metric)
          fprintf(outfile, "   %8.4f N,%9.4f W, %5.2f kilometers, %6.2f meters AMSL\n", site_x.lat, site_x.lon,
                  KM_PER_MILE * (d_x / FEET_PER_MILE), METERS_PER_FOOT * (h_x - earthradius));
        else
          fprintf(outfile, "   %8.4f N,%9.4f W, %5.2f miles, %6.2f feet AMSL\n", site_x.lat, site_x.lon, d_x / FEET_PER_MILE,
                  h_x - earthradius);
      }
...
void alloc_elev() { elev = new double[ARRAYSIZE + 10]; }
...
int main(int argc, char *argv[]) {
  ...
  for (x = 1; x <= y; x++) {
    ...
    if (strcmp(argv[x], "-dbm") == 0) dbm = 1;

    if (strcmp(argv[x], "-sdf") == 0) {
      z = x + 1;

      if (z <= y && argv[z][0] && argv[z][0] != '-') strncpy(sdf_path, argv[z], 253);
    }
    ...
    if (strcmp(argv[x], "-udt") == 0) {
      ...
      if (z <= y && argv[z][0]) {
        ...
        if (udt_file == nullptr) return ENOMEM;
    ...
    if (strcmp(argv[x], "-color") == 0) {
      ...
      if (z <= y && argv[z][0]) {
        ...
        if (color_file == nullptr) return ENOMEM;
  ...
  if (lidar) {
    ...
    if (debug) {
      fprintf(stderr, "ppd %lf, yppd %lf, %.4lf,%.4lf,%.4lf,%.4lf,%d x %d\n", ppd, yppd, max_north, min_west, min_north,
              max_west, width, height);
      fflush(stderr);
    }
    ...
    if (area_mode || topomap) {
      for (z = 0; z < txsites && z < max_txsites; z++) {
        ...
        if (max_range == 0.0) max_range = tx_range + rx_range;
  ...
  if (ppa == 0) {
    if (propmodel == 2) {  // Modl 2 = LOS
      ...
      if (cropping) {
        ...
        if (debug) {
          fprintf(stderr, "Cropping 1: max_west: %.4f cropLat: %.4f cropLon: %.4f longitude: %.5f dpp %.7f\n", max_west,
                  cropLat, cropLon, tx_site[0].lon, dpp);
          fflush(stderr);
        }
        ...
        if (debug) {
          fprintf(stderr, "Cropping 2: max_west: %.4f cropLat: %.4f cropLon: %.7f longitude: %.5f width %d\n", max_west,
                  cropLat, cropLon, tx_site[0].lon, width);
          fflush(stderr);
        }
      ...
      else if ((result = DoSigStr(mapfile, geo, kml, ngs, tx_site, txsites)) != 0)
        return result;
    }
    /*if(lidar){
            east=eastoffset;
